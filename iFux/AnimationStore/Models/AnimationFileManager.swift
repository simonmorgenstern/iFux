//
//  AnimationFileManager.swift
//  iFux
//
//  Created by Simon Morgenstern on 04.07.22.
//


import Foundation

class AnimationFileManager: ObservableObject {
    @Published var animationURLs: Array<URL> = []
    ...
    func renameFile (_ url: URL, newName: String) -> Bool {
        if let documentDirectory = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first {
            let pathWithFileName = documentDirectory.appendingPathComponent("\(newName).json")
            do {
    ...
    func copyFile(_ url: URL, name: String) -> Bool {
        if let documentDirectory = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first {
            ...
            var newURL: URL
            repeat {
                newURL = documentDirectory.appendingPathComponent("\(name)-\(counter).json")
                counter += 1
    ...
    func loadURLs () {
        ...
        do {
            ...
            self.animationURLs = filteredURLs
        } catch {
